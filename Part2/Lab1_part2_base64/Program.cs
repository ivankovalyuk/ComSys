...
namespace Lab1_part2_base64
{
    ...
    class Program
    {
       ...
       static string _alph = @"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
        static void Main(string[] args)
        {
            ...
            FileAnalyser f1Base64 = new FileAnalyser(@"C:\Users\Ivan\Desktop\Lab1_part2_base64\ShevaBase64.txt");
            FileAnalyser f2Base64 = new FileAnalyser(@"C:\Users\Ivan\Desktop\Lab1_part2_base64\PodrevlanBase64.txt");
            FileAnalyser f3Base64 = new FileAnalyser(@"C:\Users\Ivan\Desktop\Lab1_part2_base64\PCIBase64.txt");

            FileAnalyser f1Base64Bz2 = new FileAnalyser(f1Base64Compress.FullName);
            FileAnalyser f2Base64Bz2 = new FileAnalyser(f2Base64Compress.FullName);
            FileAnalyser f3Base64Bz2 = new FileAnalyser(f3Base64Compress.FullName);
            ...
            using (BinaryReader br = new BinaryReader(File.Open(pathFrom, FileMode.Open), Encoding.UTF8))
            {
                using (StreamWriter sr = new StreamWriter(File.OpenWrite(pathTo)))
                {
                    ...
                    for (int i = 0; i < br.BaseStream.Length - mod; i += 3)
                    {
                        ...
                        sr.Write(String.Format($"{ _alph[(val >> 18) & 0x3F]}{_alph[(val >> 12) & 0x3F]}{_alph[(val >> 6) & 0x3F]}{_alph[val & 0x3F]}"));
                    }
                    if (mod == 2)
                    {
                        val = ((br.ReadByte() << 8) + br.ReadByte() << 2);
                        sr.Write(String.Format($"{_alph[(val >> 12) & 0x3F]}{_alph[(val >> 6) & 0x3F]}{_alph[val & 0x3F]}="));
                    }
                    if (mod == 1)
                    {
                        val = br.ReadByte() << 4;
                        sr.Write(String.Format($"{_alph[(val >> 6) & 0x3F]}{_alph[val & 0x3F]}=="));
                    }
                }
            }
        }
